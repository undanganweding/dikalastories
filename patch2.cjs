...
const regex = /if \(providerType === 'google'\) \{([\s\S]*?)\} else if \(providerType === 'kling'\) \{/;
const newGoogleLogic = `if (providerType === 'google') {
      ...
      // To satisfy the loop outer scope logic if we break out:
      break;
    } else if (providerType === 'kling') {`;

content = content.replace(regex, newGoogleLogic);
